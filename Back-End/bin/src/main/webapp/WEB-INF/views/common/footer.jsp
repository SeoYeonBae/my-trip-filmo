...
<!-- Footer start-->
<footer class="footer text-center text-black-50"
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="">
					<div
						class="form-floating mb-1 modal-checkbox-div d-flex justify-content-end"
						id="login-saveid-div">
						<input class="form-check-input" type="checkbox" value="ok"
							id="login-saveid" name="login-saveid" ${idck} />
						<label class="form-check-label" for="login-saveid"> 아이디 저장</label>
					</div>
					<div class="form-floating mb-3">
						<input type="text" class="form-control" id="login-id"
							name="login-id" placeholder="ID" spellcheck="false"
							value="${saveid}" onkeyup="enterkeyLogin()" />
						<label for="login-id" class="form-label">ID</label>
					</div>
					<div class="form-floating mb-3">
						<input type="password" class="form-control" id="login-password"
							name="login-password" placeholder="Password"
							onkeyup="enterkeyLogin()" /> <label
							for="login-password" class="form-label">Password</label>
					</div>
				</form>
			</div>
			<!-- Modal footer -->
			<div class="modal-footer">
				<button type="button" class="btn-dark btn-sm" name="btn-user-login"
					onclick="userLogin()">로그인</button>
				<button type="button" class="btn-dark btn-sm"
					name="btn-user-login-find-pass" onclick="findPassShow()">
					비밀번호 찾기</button>
				<button type="button" class="btn-dark btn-sm"
					id="btn-user-login-reset" data-bs-dismiss="modal">
					취소</button>
			</div>
		</div>
	</div>
...
<!-- 로그인 modal end -->
<!-- 비밀번호 찾기 modal start -->
<div class="modal fade" id="findPassModal" data-bs-backdrop="static"
	data-bs-keyboard="false" tabindex="-1"
	aria-labelledby="staticBackdropLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body" id="find-pass-modal-body">
				<form action="">
					<div class="form-floating mb-3">
						<input type="text" class="form-control" id="find-pass-id"
							name="find-pass-id" placeholder="ID" spellcheck="false" />
						<label for="find-pass-id" class="form-label">ID</label>
					</div>
					<div class="form-floating mb-3">
						<input type="text" class="form-control" id="find-pass-name"
							name="find-pass-name" placeholder="Name" /> <label
							for="find-pass-name" class="form-label">Name</label>
					</div>
				</form>
			</div>
			<!-- Modal footer -->
			<div class="modal-footer" id="find-pass-modal-footer">
				<button type="button" class="btn-dark btn-sm" name="btn-find-pass"
					onclick="userFindPass()">비밀번호 찾기</button>
				<button type="button" class="btn-dark btn-sm"
					id="btn-find-pass-reset" data-bs-dismiss="modal">
					취소</button>
			</div>
		</div>
	</div>
				...
				<form id="form-join" method="post" action="">
					...
					<div class="form-floating mb-3">
						<input type="text" class="form-control" id="join-id"
							name="join-id" placeholder="ID" spellcheck="false" />
					...
					<div class="form-floating mb-3">
						<input type="password" class="form-control" id="join-password"
							name="join-password" placeholder="Password" />
						<label for="join-password" class="form-label">Password</label>
						<div class="join-check-result" id="passwordcheck-result"></div>
					</div>
					<div class="form-floating mb-3">
						<input type="password" class="form-control"
							id="join-password-check" placeholder="Password Check" />
						<label for="join-password-check" class="form-label">Confirm
							Password</label>
					</div>
					<div class="form-floating mb-3">
						<input type="text" class="form-control" id="join-name"
							name="join-name" placeholder="Name" /> <label
							for="join-name" class="form-label">Name</label>
					</div>
					<div class="form-floating mb-3">
						<input type="email" class="form-control" id="join-email"
							name="join-email" placeholder="seoseoduo@ssafy.com" />
						<label for="join-email" class="form-label">Email</label>
					</div>
					<div class="form-floating mb-3">
						<input type="tel" class="form-control" id="join-tel"
							name="join-tel" placeholder="010-1234-5678" />
					...
					<div class="form-floating mb-3 modal-checkbox-div d-flex">
						...
						<input type="checkbox" class="form-check-input"
							id="join-is-manager" name="join-is-manager" value="manager" />
						<label class="form-check-label" for="join-is-manager">Yes</label>
					</div>
					<div class="form-floating mb-3" id="join-manager-check-div"
						style="display: none">
						<input type="text" class="form-control" id="join-manager-check"
							name="join-manager-check" placeholder="manager-check" />
						<label for="join-manager-check" class="form-label">Enter
							the manager number</label>
					</div>
				</form>
			</div>
			<!-- Modal footer -->
			<div class="modal-footer">
				<button type="button" class="btn-dark btn-sm" name="btn-user-join"
					onclick="userJoin()">회원가입</button>
				<button type="button" class="btn-dark btn-sm"
					id="btn-user-join-reset" data-bs-dismiss="modal">
					취소</button>
			</div>
		</div>
	</div>
				...
				<form action="">
					<div class="form-floating mb-3">
						<input type="text" class="form-control" id="info-id"
							name="info-id" value="${userinfo.id}" readonly />
						<label for="info-id" class="form-label">ID</label>
					</div>
					<div class="form-floating mb-3">
						<input type="password" class="form-control" id="info-password"
							name="info-password" value="${userinfo.password}" />
						<label for="info-password" class="form-label">Password</label>
					</div>
					<div class="form-floating mb-3">
						<input type="password" class="form-control"
							id="info-password-check" placeholder="Password Check" />
						<label for="info-password-check" class="form-label">Confirm
							Password</label>
					</div>
					<div class="form-floating mb-3">
						<input type="text" class="form-control" id="info-name"
							name="info-name" value="${userinfo.name}" readonly />
						<label for="info-name" class="form-label">Name</label>
					</div>
					<div class="form-floating mb-3">
						<input type="email" class="form-control" id="info-email"
							name="info-email" value="${userinfo.email}" />
						<label for="info-email" class="form-label">Email</label>
					</div>
					<div class="form-floating mb-3">
						<input type="tel" class="form-control" id="info-tel"
							name="info-tel" value="${userinfo.tel}" /> <label
							for="info-tel" class="form-label">Tel</label>
					</div>
				</form>
			</div>
			<!-- Modal footer -->
			<div class="modal-footer">
				<button type="button" class="btn-dark btn-sm" name="btn-user-modify"
					onclick="userModify()">회원정보 수정</button>
				<button type="button" class="btn-dark btn-sm" name="btn-user-delete"
					onclick="userDelete()">회원 탈퇴</button>
				<button type="button" class="btn-dark btn-sm"
					id="btn-user-info-reset" data-bs-dismiss="modal">
					취소</button>
			</div>
		</div>
	</div>
...
<!-- Core theme JS-->
<script>
	var sessionCheck = sessionStorage.length;
	console.log(sessionCheck + " 로그인 상태 화긴ㅇ");
    ...
    // ***비밀번호 찾기***
        ...
        let config = {
      	...
      	fetch("${root}/user/findPass", config)
    		.then(response => response.text())
    		.then(data => {
    			if (data) { // 비밀번호 찾기 성공
    				showPass(name, data);
        		} else {
        			alert("아이디와 이름에 해당하는 사용자가 존재하지 않습니다.");
        		}
    		});
      }
 		}
		...
		function userDelete() {
			if (confirm("탈퇴하시겠습니까?")) {
        let userid = document.querySelector("#info-id").value;
        let config = {
          method: 'DELETE',
        	cache: 'no-cache',
        	headers : { "Content-Type": "application/json" },
        }
				fetch("${root}/user/" + userid, config)
	  		.then(response => response.text())
	  		.then(data => {
	  			if (data == 1) {
	  				let modal = bootstrap.Modal.getOrCreateInstance(document.getElementById('infoModal'));
