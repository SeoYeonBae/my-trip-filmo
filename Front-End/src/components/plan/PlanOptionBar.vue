<template>
  <b-container fluid>
    <b-row class="mb-4 text-center justify-content-md-center">
      <h5>나는</h5>
      <b-col md="3">
        <b-form-select v-model="selectSidoCode" :options="sidos" @change="this.makeGugun"></b-form-select>
      </b-col>
      <b-col md="3">
        <b-form-select v-model="selectGugunCode" :options="guguns" @change="this.makeList"></b-form-select>
      </b-col>
      <h5>로 떠나고 싶어요</h5>
    </b-row>
...
export default {
  ...
  methods: {
    ...
    makeContents() {
      ...
      else if (this.imageShow && this.tourList.length != 0) this.$emit("makeCard");
    },
    makeGugun() {
